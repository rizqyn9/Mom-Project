...
const EditProduct = ({data}) => {
    const [userInfo, setuserInfo] = useState({
        file:[],
        filepreview:`http://localhost:3005/${data.Images.file_name}`,
    });

    const [inputFields, setInputFields] = useState(data.Properties)
    const [title , setTitle] = useState(data.Nama_Barang)
        ...
        <div className="flex items-center justify-center w-full">
            ...
            <form onSubmit={e => handleSubmit(e)} encType='multipart/form-data' className="flex flex-col w-1/3">
                <div className="flex items-center justify-between mt-4">
                    ...
                    <input type="text" onChange={(e) => setTitle(e.target.value)} value={title}  placeholder="Judul" className="w-2/3 bg-green-200 px-3 py-1 rounded-md ring-green-300  focus:outline-none focus:bg-white focus:shadow-md focus:ring-2"/>
                </div>
                <div className="flex-auto flex items-center justify-between mt-4">
                    ...
                    <textarea type="text" onChange={(e) => setDesc(e.target.value)} value={desc} placeholder="Deskripsi" className="w-2/3 bg-green-200 px-3 py-1 rounded-md ring-green-300  focus:outline-none focus:bg-white focus:shadow-md focus:ring-2"/>
                </div>
